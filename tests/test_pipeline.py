...
def test_UseExoREM(
    ...
    def test_isFileExist(afilepath):
        assert os.path.isfile(afilepath) 

    test_isFileExist(inputfile)

    def test_isJSON(inputfile):
        assert str(inputfile)[-5:]==".json"

    test_isJSON(inputfile)
    ...
    def test_isInputFilled(R, Distance, FitType, dir_input, dir_obs, dir_output, planetName, typeCloud):
        assert len(str(R))*len(str(Distance))*len(FitType)*len(dir_input)*len(dir_obs)*len(dir_output)*len(planetName)*len(typeCloud) > 0

    test_isInputFilled(R, Distance, FitType, dir_input, dir_obs, dir_output, planetName, typeCloud)
    ...
    def test_isArrayFull(anarray):
        assert len(anarray) > 0

    test_isArrayFull(tabeGrille)
    ...
    test_isArrayFull(SED)
    test_isArrayFull(Spectrum)
    ...
    if FitType=="Full" or FitType=="SED":
        test_isFileExist(dir_obs+"Tab_name-file_filters.tsv")
    ...
    for i in range(len(loc)):
        if (FitType=="Full") or (FitType=="Spec"):
            if i<len(list(Spectrum.field("lambda"))):
                ...
                Filtre = (loadtxt("../"+str((FiltreId[FiltreId["id"]==array(SED.field("Filter"))[i-len(list(Spectrum.field("lambda")))]]["file"][0]).decode())))
                FilNN, err = ConstrFilter(Filtre[:,0],Filtre[:,1],tabWavGeneral, logfile=err)
        elif FitType=="SED":
            Filtre=(loadtxt("../"+str((FiltreId[FiltreId["id"]==array(SED.field("Filter"))[i]]["file"][0]).decode())))
            FilNN, err = ConstrFilter(Filtre[:,0],Filtre[:,1],tabWavGeneral, logfile=err)
    ...
    def test_AreSameLen(arrayone,arrayetwo):
        assert len(arrayone)==len(arrayetwo)

    test_AreSameLen(tabWavGeneral,FiltreTrans.transpose())
    ...
    for i in range(len(tab_recap)):
        table_param.array[i] = (10.**tab_recap[i,1],tab_recap[i,2],tab_recap[i,3],str(tab_recap[i,4].decode()))
